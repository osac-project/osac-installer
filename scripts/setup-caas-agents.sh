...
[[ "${AGENT_VM_MEMORY}" =~ ^[0-9]+$ ]] || { echo "ERROR: AGENT_VM_MEMORY must be numeric: ${AGENT_VM_MEMORY}" >&2; exit 1; }
[[ "${AGENT_VM_VCPUS}" =~ ^[0-9]+$ ]] || { echo "ERROR: AGENT_VM_VCPUS must be numeric: ${AGENT_VM_VCPUS}" >&2; exit 1; }
...
if command -v osac-hosts-entry >/dev/null 2>&1; then
    ...
    if [ -f "\${NM_DNSMASQ_PID}" ] && kill -0 "\$(cat "\${NM_DNSMASQ_PID}")" 2>/dev/null; then
        echo "\${CONF_ENTRY}" > "/etc/NetworkManager/dnsmasq.d/\${SLUG}.conf"
        systemctl reload NetworkManager
    else
        echo "\${CONF_ENTRY}" > "/etc/dnsmasq.d/\${SLUG}.conf"
